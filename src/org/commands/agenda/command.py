...
def register(app: typer.Typer, app_config: org.config.app.AppConfig) -> None:
    ...
    def agenda(  # noqa: PLR0913
        ...
        offset: int = typer.Option(
            0 if app_config.agenda.offset is None else app_config.agenda.offset,
            "--offset",
            metavar="N",
        ...
        date: str | None = typer.Option(
            app_config.agenda.date,
            "--date",
            metavar="DATE",
            help="Agenda start date (default: today). Formats: YYYY-MM-DD or ISO datetime",
        ),
        days: int = typer.Option(
            1 if app_config.agenda.days is None else app_config.agenda.days,
            "--days",
            metavar="N",
            min=1,
        ...
        no_completed: bool = typer.Option(
            False if app_config.agenda.no_completed is None else app_config.agenda.no_completed,
            "--no-completed",
            help="Omit tasks in completed states (including repeats)",
        ),
        no_overdue: bool = typer.Option(
            False if app_config.agenda.no_overdue is None else app_config.agenda.no_overdue,
            "--no-overdue",
            help="Omit overdue scheduled and deadline tasks",
        ),
        no_upcoming: bool = typer.Option(
            False if app_config.agenda.no_upcoming is None else app_config.agenda.no_upcoming,
            "--no-upcoming",
            help="Omit upcoming deadlines",
        ),
        future_repeats: bool = typer.Option(
            True if app_config.agenda.future_repeats is None else app_config.agenda.future_repeats,
            "--future-repeats/--no-future-repeats",
            help="Include potential future planning repeats in agenda days",
        ),
        view: str | None = typer.Option(
            app_config.agenda.view,
            "--view",
            metavar="NAME",
        ...
        org.logging.log_command_config(app_config, "agenda")
