...
** More Rich support & other output fixes
*** General updates
- derive the size of the viewport from the console window

*** Stats layout
- derive the --buckets property from the viewport size
- top frequency value to be moved to top in the middle of the dates
- columnar display on the stats summary - tasks section should be to the side of the histograms
- use boxes for the sections
- Don't indent TAGS and GROUPS sections on summary

*** Task list scroll
- scroll through task list
- unicode strings in heading cause misalignment of tags on task lists

*** Bugfixes
- There are cases still that print =None= and Python class strings (stream of lists of values) - should be mended with a custom formatter for collections & values.

** Mutable parse tree
*** Use mutable orgparse
