...
** Update todo-keys & done-keys based on file contents
OrgRootNode has an environment that stores the `todo_keys` and `done_keys` properties. Use that to update the relevant settings after loading all the files.
** Output colors
