...
- none handling
- display results in org mode syntax highlighting
- sort_by should orderd top to bottom
- initial nodes stream handling (root vs list of nodes)
- stream folding into an array syntax ~[<subquery>]~ - must not be preceeded by a subquery or an identity
- sum function
- ~as $variable~ syntax for defining variables
