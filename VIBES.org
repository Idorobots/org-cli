...
:plan: [[file:plans/query-language1.md]]
:END:
...
- ~.[N]~ - N-th element of a collection
- ~.[2:5]~ - collection slice (indices 2, 3, 4)
- ~,~ - Combine filters into one stream (produces a tuple of results)
- ~|~ - Pipe output to another filter
- ~select(...)~ - Select elements by condition
- ~sort_by(...)~ - Sort collection by value selected by the inner query

The field access form should support accessing any fields of the ~OrgNode~, ~OrgDate~ and any intermediate collections, string, numbers. If a field does not exist on an object, an exception must be raised and handled by typer (ideally in a nicely formatted fashion).

Forms can be combined into more complex invocations, for example ~.children[0].todo~ should return a todo state of the first child of a node. Multiple forms can be piped together to form a sequential filter, for example ~.children[0] | .todo~ should return a todo state of the first child of a node.
...
- ~>~ - compares numeric values as ~a > b~
- ~<~ - compares numeric values as ~a < b~
- ~>=~ - compares numeric values as ~a >= b~
- ~<=~ - compares numeric values as ~a <= b~
- ~==~ - compares any values as equal (structural equality)
- ~!=~ - compares any values as not equal (structural equality)
- ~matches~ - regex matches string, ~a matches b~ should be equivalent to  ~re.combile(b).match(a)~

Type mismatches in operator invocations should be raised as exeptions.
