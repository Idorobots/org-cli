...
:END:
*** Filtering, ordering & enrichments
- [ ] Use it for repeated task filtering - =select(((.repeats | map(.after)) + .todo)[] in $done_keys) | .repeats = [.repeats[] | select(.after in $done_keys)]=
- [X] Use it for property additions
- [ ] Use it for numeric gamify exp

*** Query language mutation
- ~a = b~ assignment operator
- ~+=~ append
- ~-=~ remove
- ~properties.foo = none~ set/delete property
