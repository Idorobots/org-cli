...
- Add a way to dynamically define new variables via the ~<subquery> as $variable~ syntax. A variable defined this way, should be accessible further down the processing pipeline, i.e. the =|= operator encountering a new variable definition on the left side, should bake it available on the right side. For example: =. as $root | $root[], ($root.children | length)= results in a steam of tuples of the root node along with its children.


- stream folding into an array syntax ~[<subquery>]~ - must not be preceeded by a subquery or an identity

**** Query language bugfixes                                                                   :Bug:
- none handling
